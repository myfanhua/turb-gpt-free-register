...
// Export helpers needed for so-token: sessionObserverToken expects a *flow string*
// key (e.g. oauth_create_account), after se(flow, challenge) arms the collector.
const EXPOSE_REPLACEMENT =
  "return o?r?.[n(63)]?ce({so:o,c:r[n(63)]},t):o:null},t.token=ye,t.__debug_n=_n,t.__debug_bindProof=D,t.__debug_se=se,t.__debug_re=re,t}({});";
...
function createElement(tagName) {
  ...
  return {
    ...
    getContext(type) {
      ...
      if (t.includes("webgl")) {
        const p = globalThis.__sentinel_payload || {};
        return {
          getExtension() {
            return { UNMASKED_VENDOR_WEBGL: 37445, UNMASKED_RENDERER_WEBGL: 37446 };
          },
          getParameter(name) {
            if (Number(name) === 37445) return String(p.webgl_vendor || "Google Inc. (Intel)");
            if (Number(name) === 37446) return String(p.webgl_renderer || "ANGLE (Intel)");
            return "";
          },
        };
      }
...
function installRuntime(payload) {
  globalThis.__sentinel_payload = payload;
  const seedText = String(payload.profile_seed || payload.profile_id || payload.device_id || "");
  let seedNum = 0;
  for (let i = 0; i < seedText.length; i += 1) seedNum = (seedNum * 33 + seedText.charCodeAt(i)) >>> 0;
  const screen = {
  ...
  const performance = {
    now: () => Number(payload.performance_now || (10000 + (seedNum % 90000) + 0.67)),
    timeOrigin: Number(payload.time_origin || (1710000000000 + (seedNum % 86400000))),
  ...
  globalThis.navigator = {
    ...
    platform: String(payload.navigator_platform || payload.platform || "Win32"),
    vendor: "Google Inc.",
    webdriver: false,
  };
  globalThis.Intl = globalThis.Intl || {};
  globalThis.Intl.DateTimeFormat = function DateTimeFormat() {
    return {
      resolvedOptions() {
        return { timeZone: String(payload.timezone || "Asia/Tokyo") };
      },
    };
  };
  globalThis.location = {
  ...
  // CRITICAL: do NOT run delayed timers immediately.
  // Sentinel SO collector uses setTimeout(..., 60000) as a real timeout; a sync
  // polyfill rejects Nt() instantly and so_token stays empty forever.
  ...
  globalThis.setTimeout = (cb, ms, ...args) => {
    ...
    // Extremely constrained environments: only run zero-delay callbacks.
    if (delay <= 0) {
      try {
        cb(...args);
      } catch (_) {}
  ...
  globalThis.clearTimeout = (id) => {
    ...
    if (handle !== undefined && _nativeClearTimeout) _nativeClearTimeout(handle);
    _timers.delete(id);
  };
  ...
  globalThis.clearInterval = (id) => {
    const handle = _timers.get(id);
    if (handle !== undefined && _nativeClearInterval) _nativeClearInterval(handle);
    _timers.delete(id);
  };
  ...
  // Force UA into payload value even if host Node overrides navigator later.
  try {
    Object.defineProperty(globalThis.navigator, "userAgent", {
      configurable: true,
      get() {
        return String(payload.user_agent || "Mozilla/5.0");
      },
    });
  } catch (_) {}
...
async function run(payload, sdkSource) {
  ...
  if (payload.action === "solve") {
    ...
    const flow = String(payload.flow || challenge.flow || "authorize_continue").trim() || "authorize_continue";

    // Mirror browser token() order:
    // 1) arm session-observer collector with se(flow, challenge)
    // 2) getEnforcementToken(challenge) for p
    // 3) sessionObserverToken(flow) for so  (NOTE: arg is flow string, not challenge)
    try {
      if (typeof globalThis.SentinelSDK.__debug_se === "function") {
        globalThis.SentinelSDK.__debug_se(flow, challenge);
      }
    } catch (_) {}
    ...
    let soValue = null;
    let soRaw = null;
    const soRequired = !!(challenge && challenge.so && challenge.so.required);
    const soAttempts = soRequired ? 10 : 2;
    for (let i = 0; i < soAttempts && !soValue; i += 1) {
      try {
        if (typeof globalThis.SentinelSDK.sessionObserverToken === "function") {
          // Critical: SDK looks up ne.get(flow); passing the challenge object always misses.
          soRaw = await globalThis.SentinelSDK.sessionObserverToken(flow);
        }
      } catch (_) {
        soRaw = null;
      }
      if (soRaw && typeof soRaw === "object") {
        soValue = soRaw.so || soRaw.s || null;
      } else if (typeof soRaw === "string" && soRaw.length > 8) {
        soValue = soRaw;
      }
      if (!soValue && i + 1 < soAttempts) {
    ...
    let bindResult = null;
    try {
      bindResult = globalThis.SentinelSDK.__debug_bindProof(challenge, requestP);
    } catch (_) {
      bindResult = null;
    }
    if (!soValue && bindResult && typeof bindResult === "object") {
      soValue = bindResult.so || bindResult.s || null;
    } else if (!soValue && typeof bindResult === "string") {
      soValue = bindResult;
    }
    ...
    let tValue = null;
    try {
      tValue = dx ? await globalThis.SentinelSDK.__debug_n(challenge, dx) : null;
    } catch (_) {
      tValue = null;
    }
    if (tValue === 0 || tValue === "0") tValue = "0";
    return { final_p: finalP, t: tValue, so: soValue, flow };
  }
