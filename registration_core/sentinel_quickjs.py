...
def _ensure_sdk_file(session: Any, timeout_ms: int) -> Path:
    ...
    with _SDK_CACHE_LOCK:
        ...
        temporary_file = sdk_file.with_name(f"{sdk_file.name}.{uuid.uuid4().hex}.tmp")
        try:
            temporary_file.write_bytes(content)
            temporary_file.replace(sdk_file)
        finally:
...
def get_sentinel_token_bundle_via_quickjs(
    ...
    *,
    ...
    """Try the QuickJS path and return both Sentinel headers.

    Return shape:
      {"token": "{p,t,c,id,flow}", "so_token": "{so,c,id,flow}"}

    `so_token` is only present when the server challenge exposes `so` data;
    this is required by the current about_you/create_account flow captured from
    the browser.
    """
    log = log or (lambda m: logger.info(m))
    quickjs_script = _quickjs_script_path()
    if not quickjs_script.exists():
        log(f"Sentinel QuickJS ?????: {quickjs_script}")
        return None
    ...
    try:
        ...
        requirements = _run_quickjs_action(
            ...
            payload={"device_id": did, **_sentinel_profile_payload(browser_profile)},
            timeout_ms=timeout_ms,
        )
        request_p = str(requirements.get("request_p") or "").strip()
        if not request_p:
            log("Sentinel QuickJS ??: requirements ??? request_p")
            return None
        ...
        if not c_value:
            log("Sentinel QuickJS ??: challenge token ??")
            return None
        ...
        solved = _run_quickjs_action(
            ...
            payload={
                "device_id": did,
                **_sentinel_profile_payload(browser_profile),
                "request_p": request_p,
                "challenge": challenge,
                # sessionObserverToken / se() are keyed by flow string.
                "flow": str(flow or "authorize_continue"),
            },
            timeout_ms=timeout_ms,
        )
        final_p = str(solved.get("final_p") or solved.get("p") or "").strip()
        if not final_p:
            log("Sentinel QuickJS ??: solve ??? final_p")
            return None

        t_raw = solved.get("t")
        t_value = "" if t_raw is None else str(t_raw).strip()
        if not t_value:
            log("Sentinel QuickJS ??: solve ????? t")
            return None

        so_raw = solved.get("so")
        so_value = "" if so_raw is None else str(so_raw).strip()
        token = json.dumps(
        ...
        log(
            f"Sentinel QuickJS ?? (p_len={len(final_p)} t_len={len(t_value)} "
            f"so_len={len(so_value)} c_len={len(c_value)})"
        )
        return {"token": token, "so_token": so_token}
    except Exception as e:
        log(f"Sentinel QuickJS ??: {e}")
        return None
