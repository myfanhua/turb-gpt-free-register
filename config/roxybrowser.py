...
# 首次打开登录页遇到代理断流/渲染器超时时，回收当前临时环境并换新会话重试。
ROXY_STARTUP_MAX_ATTEMPTS: int = 2
ROXY_STARTUP_RETRY_DELAY: int = 2
