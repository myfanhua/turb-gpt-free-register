...
# 免手机验证 synthetic auth 转化（学习自 codex-auth-helper）：
#   True  = 注册成功后自动做本地 synthetic 转化（构造 alg=none id_token + auth.json），
#           跳过需要手机验证的 Codex OAuth 流程；纯本地零网络请求。
#   False = 走原有 Codex OAuth 接码流程（受 ENABLE_CODEX_AUTO 控制）。
# 注意：synthetic auth.json 的 refresh_token 是 sessionToken 占位，access token
# 过期后无法自动刷新，需要重新转化；这是该方案的固有限制。
CODEX_SYNTHETIC_AUTH_ENABLE: bool = False
