# -*- coding: utf-8 -*-
"""纯 Python Turnstile token 求解器。

移植自 chatgpt2api（MIT License, Copyright (c) 2026 kunkun）
`utils/turnstile.py`：把 chat-requirements 返回的 ``dx``（base64 + XOR 混淆的
指令流）在进程内解释执行，产出 ``OpenAI-Sentinel-Turnstile-Token``。
无需浏览器、无需第三方依赖；求解失败返回 None，由调用方决定降级策略。
"""
...
def solve_turnstile_token(dx: str, p: str) -> Optional[str]:
    """解释执行 dx 指令流；p 必须是发送给 prepare 的同一个 p token。"""
    try:
    ...
    # 新版 dx 会套娃：主指令流末尾把 process_map[9] 换成二层指令流，
    # 必须继续执行嵌套流（可能多级）才能拿到最终结果；最多 5 层防御死循环。
    current = token_list
    for _pass in range(5):
        if result:
            break
        before = process_map.get(9)
        for token in current:
            try:
                ...
                continue
        after = process_map.get(9)
        if isinstance(after, list) and after is not before and after != before:
            current = after
        else:
            break
